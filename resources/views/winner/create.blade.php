...
@section('page-title', 'Create Winner')
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div class="min-w-0 flex-1">
            ...
            <p class="mt-1 text-sm text-gray-500">
                Lock a winning variant for specific targeting criteria
            </p>
        </div>
        <div class="mt-4 flex md:ml-4 md:mt-0">
            <a href="{{ route('remote-config.winners.index') }}" class="inline-flex items-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                Cancel
            </a>
        </div>
    </div>

    <form method="POST" action="{{ route('remote-config.winners.store') }}" class="space-y-6">
        @csrf
        <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-full">
                        <div class="flex items-center">
                            <input id="is_active" name="is_active" type="checkbox" checked class="h-4 w-4 rounded border-2 border-gray-400 text-primary-600 focus:ring-primary-600">
            ...
            <div class="flex items-center justify-end gap-x-6 border-t border-gray-900/10 px-4 py-4 sm:px-8">
                <a href="{{ route('remote-config.winners.index') }}" class="text-sm font-semibold text-gray-900">Cancel</a>
                <button type="submit" class="rounded-md bg-primary-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-500">Create Winner</button>
            </div>
        </div>
    </form>
</div>
