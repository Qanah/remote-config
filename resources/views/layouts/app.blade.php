...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - {{ config('remote-config.admin.title', 'Remote Config & Experiments') }}</title>
...
<body class="h-full bg-gray-50 font-sans antialiased">
            ...
            <div class="fixed inset-0 flex">
                <div x-show="sidebarOpen"
                    ...
                    <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-white px-6 pb-2">
                        @include('remote-config::partials.sidebar-content')
                    </div>
                </div>
            </div>
        </div>
    ...
    @if(session('success'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed bottom-4 right-4 z-50">
    ...
    @if(session('warning'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed bottom-4 right-4 z-50">
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 shadow-lg max-w-md">
                <div class="flex items-start">
                    ...
                    <button @click="show = false" class="ml-auto pl-3">
                        <svg class="h-4 w-4 text-yellow-400 hover:text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                </div>
